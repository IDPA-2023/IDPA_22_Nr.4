		...
		try {
			...
			for (let i = 0; i < userGroup.items.length; i++) {
				...
				const user = serializeNonPOJOs<User>(
					await locals.pb.collection('users').getOne(userGroup.items[i].userIDFS)
				);
				users.push(user);
			}
			return {
	...
	deleteUser: async ({ request, locals, params }) => {
		...
		try {
			const record = await locals.pb
				...
				.getFirstListItem(`userIDFS="${data.userId}" && groupIDFS="${params.groupId}"`);
	...
	editGroup: async ({ request, locals, params }) => {
		...
		try {
			if (locals.user) {
				...
				const record = await locals.pb
					.collection('group')
					.update(params.groupId as string, groupData);
				...
				for (let i = 0; i < users.length; i++) {
					...
					const record = await locals.pb
						.collection('userGroup')
						.update(userGroups[i].id as string, userGroupData);
				}
			}
		} catch (err) {
			console.log(err);
