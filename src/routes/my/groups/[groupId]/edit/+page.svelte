...
<div class="w-full">
	<form action="?/editGroup" method="post" class="flex flex-col">
		...
		{#if data.userData}
							...
							<div class="w-fit flex ml-auto">
								<div class="flex w-fit flex-col mr-2">
									...
									<input
										...
										name={'weight' + idx}
										id={'weight' + idx}
										value={data.userData.userGroups[idx].weight}
									/>
								</div>
								<form action="?/deleteUser" method="post" class="flex">
									<input type="hidden" name="userId" value={user.id} />
									<button class="btn btn-secondary btn-circle w-12 p-2 ml-2 mb-auto mt-auto"
			...
			{/each}
		{/if}
		<button type="submit" class="btn-primary btn w-fit ml-auto mr-auto mt-5"
