...
export const load: PageServerLoad = ({ locals, params }) => {
	...
	const getQuestion = async (questionId : string) => {
		try {
			const question = serializeNonPOJOs(await locals.pb.collection('question').getOne(questionId as string, {'$autoCancel': false}))
    ...
    const getOptions = async (questionId : string) => {
        try {
			const question = serializeNonPOJOs<Question>(await locals.pb.collection('question').getOne(questionId as string, {'$autoCancel': false}))
			console.log(question)
            if((question.type === "select") || (question.type === "multiple")){
				const resultList = serializeNonPOJOs(await locals.pb.collection('option').getList(1, 50, {
...
export const actions : Actions = {
	editQuestion:async ({ locals, request, params }) => {
		...
		try {
			...
			const record = await locals.pb.collection('question').update(params.questionId as string, questionData)

			if((record.type === "multiple") || (record.type === "select")){
				if(data.options && (data.options.length != 0)){
					let usedIndexes = []
					const oldOptions = serializeNonPOJOs(await locals.pb.collection('option').getList(1, 50, {
					...
					for(let i=0; i<oldOptions.items.length; i++){
						if(!data[oldOptions.items[i].id]){
							...
							if(oldOptions.items[i].option !== data['option'+data[oldOptions.items[i].id]]){
								...
								const response = await locals.pb.collection('option').update(oldOptions.items[i].id, newOptionData)
							}
