...
{#if (data.question.question.type === "multiple") || (data.question.question.type) === "select"}
    ...
    <form action="?/editQuestion" method="post">
        <div class="w-96 ml-auto mr-auto flex flex-col">
            <Input label="Frage" id="question" name="question" value={data.question.question.question}/>
            ...
            <div class="w-80 flex mb-2 ml-auto mr-auto">
                ...
                <button type="button" on:click={() => removeOptionByIndex(idx)} class="btn btn-secondary mt-auto ml-1 w-12"><Icon src={Trash}/></button>
            </div>
            {/each}
            ...
            <button type="button" on:click={addOption} class="btn btn-secondary ml-auto mr-auto">Option hinzufügen</button>
            <button type="submit" class="btn btn-primary mt-7 w-56 ml-auto mr-auto">Änderungen Speichern</button>
        </div>
    </form>
